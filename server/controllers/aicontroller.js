...
export const generateImage = async (req, res) => {
  ...
  ,${prompt},${secure_url},'image',${publish ? true : false})`;
...
export const removeImageObject = async (req, res) => {
  try {
    ...
    const  image  = req.file; // multer add file (image) to req
    const plan = req.plan;
...
export const resumeReview = async (req, res) => {
  try {
    ...
    const prompt = `Review the following resume and provide the constructive feedback on its strengths ,weaknesses and areas for improvement.Resume Content:\n\n${pdfData.text}`;
